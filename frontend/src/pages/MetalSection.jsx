...
const MetalSection = ({ metals = [], viewMode, onAddClick }) => {
  ...
  const dummyMetals = [
    ...
    },
    {
      ...
      image: "", // or "/placeholder.svg"
      isActive: true,
      variants: [{ name: "Sterling", purity: 92.5, price: 65 }],
    },
  ];
  ...
  useEffect(() => {
    if (metals.length === 0) {
      setMetalList(
        dummyMetals.map((m) => ({ ...m, isActive: m.isActive ?? true }))
      );
    } else {
  ...
  const handleConfirmToggle = () => {
    ...
    setMetalList((prev) =>
      prev.map((metal) =>
        metal.id === metalId ? { ...metal, isActive: !metal.isActive } : metal
      )
    );
  ...
  const handleUpdateMetal = (updatedMetal) => {
    setMetalList((prev) =>
      prev.map((m) =>
        m.id === updatedMetal.id || m._id === updatedMetal._id
          ? updatedMetal
          : m
      )
    );
      ...
      {confirmToggle.show && (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-6 rounded-md shadow-md z-50">
          <div className="bg-white p-6 rounded-md shadow-md w-80">
            ...
            <p className="mb-4">
              Are you sure you want to change the status of this metal?
            </p>
